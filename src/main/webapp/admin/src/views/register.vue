  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(255, 250, 250, 0.57);borderRadius:10px">
        <h1 class="h1" style="color:rgba(81, 163, 81, 1);fontSize:28px;">药品管理系统注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			<!-- <div v-if="tableName=='xiaoshouyuan'" class="input-group">
			   <div class="label">工号</div>
			   <div class="input-container">
			     <input v-model="ruleForm.gonghao" class="input" type="text" placeholder="工号">
			   </div>
			 </div> -->
			<el-form-item label="工号" class="input" v-if="tableName=='xiaoshouyuan'">
			...
			<el-form-item label="密码" class="input" v-if="tableName=='xiaoshouyuan'">
			...
			<el-form-item label="姓名" class="input" v-if="tableName=='xiaoshouyuan'">
			...
			<el-form-item label="手机" class="input" v-if="tableName=='xiaoshouyuan'">
			...
			<el-form-item label="邮箱" class="input" v-if="tableName=='xiaoshouyuan'">
			...
			<el-form-item label="身份证" class="input" v-if="tableName=='xiaoshouyuan'">
...
export default {
  ...
  methods: {
    ...
    // 注册
    login() {
      if((!this.ruleForm.gonghao) && `xiaoshouyuan` == this.tableName){
        this.$message.error(`工号不能为空`);
        return
      }
      if((!this.ruleForm.mima) && `xiaoshouyuan` == this.tableName){
        this.$message.error(`密码不能为空`);
        return
      }
      if((!this.ruleForm.shouji) && `xiaoshouyuan` == this.tableName){
        this.$message.error(`手机不能为空`);
        return
      }
      if(`xiaoshouyuan` == this.tableName && this.ruleForm.shouji&&(!this.$validate.isMobile(this.ruleForm.shouji))){
        this.$message.error(`手机应输入手机格式`);
        return
      }
      if(`xiaoshouyuan` == this.tableName && this.ruleForm.youxiang&&(!this.$validate.isEmail(this.ruleForm.youxiang))){
        this.$message.error(`邮箱应输入邮件格式`);
        return
      }
      if((!this.ruleForm.shenfenzheng) && `xiaoshouyuan` == this.tableName){
        this.$message.error(`身份证不能为空`);
        return
      }
      if(`xiaoshouyuan` == this.tableName && this.ruleForm.shenfenzheng&&(!this.$validate.checkIdCard(this.ruleForm.shenfenzheng))){
